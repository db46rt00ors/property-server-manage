<template>
  <a-modal
    width="50%"
    :title="`退款【${ documentrefundobj.record === null ? '无' : documentrefundobj.record.property_name }】单据`"
    v-model="documentrefundobj.visible"
    @ok="handleOk"
    :destroyOnClose="true"
    okText="确认退款"
  >
    <a-row>
      对应收款单号： {{ documentrefundobj.record === null ? '无' : documentrefundobj.record.receipt_number,
      }} 收款方式：现金 作废日期： {{ invalidDate }}
    </a-row>
    <a-row>
      房号： {{
        documentrefundobj.record === null ? '无' : documentrefundobj.record.property_name
      }} 业主名称：{{
        documentrefundobj.record === null ? '无' : documentrefundobj.record.owner
      }} 收据号：{{
        documentrefundobj.record === null ? '无' : documentrefundobj.record.receipt_number
      }} 发票号：{{
        documentrefundobj.record === null ? '无' : documentrefundobj.record.invoice_number
      }}
    </a-row>
    <a-row>
